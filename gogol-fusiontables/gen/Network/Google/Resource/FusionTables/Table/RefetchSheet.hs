...
data TableRefetchSheet = TableRefetchSheet'
    { _trsTableId :: !Text
    , _trsFields :: !(Maybe Text)
...
trsFields :: Lens' TableRefetchSheet (Maybe Text)
trsFields
  = lens _trsFields (\ s a -> s{_trsFields = a})
...
instance GoogleRequest TableRefetchSheet where
        ...
        requestClient TableRefetchSheet'{..}
          = go _trsTableId _trsFields (Just AltJSON)
              fusionTablesService
          where go
                  = buildClient

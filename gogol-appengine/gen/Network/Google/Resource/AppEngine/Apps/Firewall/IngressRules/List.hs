...
--
-- /See:/ <https://cloud.google.com/appengine/docs/admin-api/ Google App Engine Admin API Reference> for @appengine.apps.firewall.ingressRules.list@.
module Network.Google.Resource.AppEngine.Apps.Firewall.IngressRules.List
    (
    -- * REST Resource
...
afirlPp :: Lens' AppsFirewallIngressRulesList Bool
afirlPp = lens _afirlPp (\ s a -> s{_afirlPp = a})
...
-- | OAuth bearer token.
afirlBearerToken :: Lens' AppsFirewallIngressRulesList (Maybe Text)
afirlBearerToken
  = lens _afirlBearerToken
      (\ s a -> s{_afirlBearerToken = a})
...
afirlFields :: Lens' AppsFirewallIngressRulesList (Maybe Text)
afirlFields
  = lens _afirlFields (\ s a -> s{_afirlFields = a})

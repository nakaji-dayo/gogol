...
-- also be updated.
--
-- /See:/ <https://cloud.google.com/appengine/docs/admin-api/ Google App Engine Admin API Reference> for @appengine.apps.authorizedCertificates.patch@.
module Network.Google.Resource.AppEngine.Apps.AuthorizedCertificates.Patch
    (
    -- * REST Resource
...
data AppsAuthorizedCertificatesPatch = AppsAuthorizedCertificatesPatch'
    ...
    , _aacpBearerToken :: !(Maybe Text)
    , _aacpAppsId :: !Text
    , _aacpFields :: !(Maybe Text)
...
aacpPp :: Lens' AppsAuthorizedCertificatesPatch Bool
aacpPp = lens _aacpPp (\ s a -> s{_aacpPp = a})
...
-- | OAuth bearer token.
aacpBearerToken :: Lens' AppsAuthorizedCertificatesPatch (Maybe Text)
aacpBearerToken
  = lens _aacpBearerToken
      (\ s a -> s{_aacpBearerToken = a})
...
aacpFields :: Lens' AppsAuthorizedCertificatesPatch (Maybe Text)
aacpFields
  = lens _aacpFields (\ s a -> s{_aacpFields = a})

...
aupcFields :: Lens' AccountsUserPermissionsCreate (Maybe Text)
aupcFields
  = lens _aupcFields (\ s a -> s{_aupcFields = a})
...
instance GoogleRequest AccountsUserPermissionsCreate
        ...
        requestClient AccountsUserPermissionsCreate'{..}
          = go _aupcParent _aupcFields (Just AltJSON)
              _aupcPayload
              tagManagerService
          where go
                  = buildClient

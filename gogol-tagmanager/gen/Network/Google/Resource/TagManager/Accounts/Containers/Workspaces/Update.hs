...
acwuFields :: Lens' AccountsContainersWorkspacesUpdate (Maybe Text)
acwuFields
  = lens _acwuFields (\ s a -> s{_acwuFields = a})
...
instance GoogleRequest
        ...
        requestClient AccountsContainersWorkspacesUpdate'{..}
          = go _acwuPath _acwuFingerprint _acwuFields
              (Just AltJSON)
              _acwuPayload
              tagManagerService
          where go

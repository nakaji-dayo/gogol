...
acedFields :: Lens' AccountsContainersEnvironmentsDelete (Maybe Text)
acedFields
  = lens _acedFields (\ s a -> s{_acedFields = a})
...
instance GoogleRequest
        ...
        requestClient
          AccountsContainersEnvironmentsDelete'{..}
          = go _acedPath _acedFields (Just AltJSON)
              tagManagerService
          where go
                  = buildClient
                      (Proxy ::

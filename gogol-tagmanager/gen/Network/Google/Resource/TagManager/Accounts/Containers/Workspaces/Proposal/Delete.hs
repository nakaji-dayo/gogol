...
data AccountsContainersWorkspacesProposalDelete = AccountsContainersWorkspacesProposalDelete'
    { _acwpdPath :: !Text
    , _acwpdFields :: !(Maybe Text)
...
acwpdFields :: Lens' AccountsContainersWorkspacesProposalDelete (Maybe Text)
acwpdFields
  = lens _acwpdFields (\ s a -> s{_acwpdFields = a})
...
instance GoogleRequest
        ...
        requestClient
          AccountsContainersWorkspacesProposalDelete'{..}
          = go _acwpdPath _acwpdFields (Just AltJSON)
              tagManagerService
          where go
                  = buildClient
                      (Proxy ::

...
data AccountsContainersWorkspacesSync = AccountsContainersWorkspacesSync'
    { _acwsPath :: !Text
    , _acwsFields :: !(Maybe Text)
...
acwsFields :: Lens' AccountsContainersWorkspacesSync (Maybe Text)
acwsFields
  = lens _acwsFields (\ s a -> s{_acwsFields = a})
...
instance GoogleRequest
        ...
        requestClient AccountsContainersWorkspacesSync'{..}
          = go _acwsPath _acwsFields (Just AltJSON)
              tagManagerService
          where go
                  = buildClient
                      (Proxy ::

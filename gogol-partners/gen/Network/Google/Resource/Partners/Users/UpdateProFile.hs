...
uupfPp :: Lens' UsersUpdateProFile Bool
uupfPp = lens _uupfPp (\ s a -> s{_uupfPp = a})
...
-- | OAuth bearer token.
uupfBearerToken :: Lens' UsersUpdateProFile (Maybe Text)
uupfBearerToken
  = lens _uupfBearerToken
      (\ s a -> s{_uupfBearerToken = a})
...
uupfFields :: Lens' UsersUpdateProFile (Maybe Text)
uupfFields
  = lens _uupfFields (\ s a -> s{_uupfFields = a})
...
instance GoogleRequest UsersUpdateProFile where
        ...
        requestClient UsersUpdateProFile'{..}
          = go _uupfXgafv _uupfUploadProtocol (Just _uupfPp)
              _uupfAccessToken
              _uupfUploadType

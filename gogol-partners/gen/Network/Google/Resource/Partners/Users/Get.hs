...
ugPp :: Lens' UsersGet Bool
ugPp = lens _ugPp (\ s a -> s{_ugPp = a})
...
-- | OAuth bearer token.
ugBearerToken :: Lens' UsersGet (Maybe Text)
ugBearerToken
  = lens _ugBearerToken
      (\ s a -> s{_ugBearerToken = a})

...
ucPp :: Lens' UpdateCompanies Bool
ucPp = lens _ucPp (\ s a -> s{_ucPp = a})
...
-- | OAuth bearer token.
ucBearerToken :: Lens' UpdateCompanies (Maybe Text)
ucBearerToken
  = lens _ucBearerToken
      (\ s a -> s{_ucBearerToken = a})
...
ucFields :: Lens' UpdateCompanies (Maybe Text)
ucFields = lens _ucFields (\ s a -> s{_ucFields = a})

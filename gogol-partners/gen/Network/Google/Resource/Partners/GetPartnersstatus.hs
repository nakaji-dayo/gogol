...
gpPp :: Lens' GetPartnersstatus Bool
gpPp = lens _gpPp (\ s a -> s{_gpPp = a})
...
-- | OAuth bearer token.
gpBearerToken :: Lens' GetPartnersstatus (Maybe Text)
gpBearerToken
  = lens _gpBearerToken
      (\ s a -> s{_gpBearerToken = a})
...
gpFields :: Lens' GetPartnersstatus (Maybe Text)
...
instance GoogleRequest GetPartnersstatus where
        ...
        requestClient GetPartnersstatus'{..}
          = go _gpXgafv _gpUploadProtocol (Just _gpPp)
              _gpAccessToken
              _gpUploadType

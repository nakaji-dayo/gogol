...
ulPp :: Lens' UpdateLeads Bool
ulPp = lens _ulPp (\ s a -> s{_ulPp = a})
...
-- | OAuth bearer token.
ulBearerToken :: Lens' UpdateLeads (Maybe Text)
ulBearerToken
  = lens _ulBearerToken
      (\ s a -> s{_ulBearerToken = a})
...
ulFields :: Lens' UpdateLeads (Maybe Text)

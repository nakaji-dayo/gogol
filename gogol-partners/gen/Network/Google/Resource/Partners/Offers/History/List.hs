...
ohlPp :: Lens' OffersHistoryList Bool
ohlPp = lens _ohlPp (\ s a -> s{_ohlPp = a})
...
-- | OAuth bearer token.
ohlBearerToken :: Lens' OffersHistoryList (Maybe Text)
ohlBearerToken
  = lens _ohlBearerToken
      (\ s a -> s{_ohlBearerToken = a})
...
ohlFields :: Lens' OffersHistoryList (Maybe Text)
ohlFields
  = lens _ohlFields (\ s a -> s{_ohlFields = a})

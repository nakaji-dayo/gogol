...
--
-- /See:/ 'examToken' smart constructor.
data ExamToken = ExamToken'
    { _etToken :: !(Maybe Text)
    , _etExamId :: !(Maybe (Textual Int64))
    , _etExamType :: !(Maybe ExamTokenExamType)
    } deriving (Eq,Show,Data,Typeable,Generic)

-- | Creates a value of 'ExamToken' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'etToken'
...
etToken :: Lens' ExamToken (Maybe Text)
etToken = lens _etToken (\ s a -> s{_etToken = a})

-- | The id of the exam the token is for.
etExamId :: Lens' ExamToken (Maybe Int64)
etExamId
  = lens _etExamId (\ s a -> s{_etExamId = a}) .
      mapping _Coerce

-- | The type of the exam the token belongs to.
etExamType :: Lens' ExamToken (Maybe ExamTokenExamType)
etExamType
  = lens _etExamType (\ s a -> s{_etExamType = a})

instance FromJSON ExamToken where
        parseJSON
          = withObject "ExamToken"
              (\ o ->
                 ExamToken' <$>
                   (o .:? "token") <*> (o .:? "examId") <*>
                     (o .:? "examType"))

instance ToJSON ExamToken where
        toJSON ExamToken'{..}
          = object
              (catMaybes
                 [("token" .=) <$> _etToken,
                  ("examId" .=) <$> _etExamId,
                  ("examType" .=) <$> _etExamType])

...
data ProjectsGetServiceAccount = ProjectsGetServiceAccount'
    { _pgsaProjectId :: !Text
    , _pgsaFields :: !(Maybe Text)
...
pgsaFields :: Lens' ProjectsGetServiceAccount (Maybe Text)
pgsaFields
  = lens _pgsaFields (\ s a -> s{_pgsaFields = a})
...
instance GoogleRequest ProjectsGetServiceAccount
        ...
        requestClient ProjectsGetServiceAccount'{..}
          = go _pgsaProjectId _pgsaFields (Just AltJSON)
              bigQueryService
          where go
                  = buildClient

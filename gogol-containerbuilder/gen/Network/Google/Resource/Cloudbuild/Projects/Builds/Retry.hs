...
--
-- Creates a new build based on the given build. This API creates a new
-- build using the original build request, which may or may not result in
-- an identical build. For triggered builds: * Triggered builds resolve to
-- a precise revision, so a retry of a triggered build will result in a
...
-- retried build will build from the tip of that branch, which may not be
-- the same revision as the original build. * If the original build
...
-- original build pulled source from Cloud Storage without specifying the
...
-- build pulled source from Cloud Storage and specified the generation of
-- the object, the new build will attempt to use the same object, which may
-- or may not be available depending on the bucket\'s lifecycle management
-- settings.
--
-- /See:/ <https://cloud.google.com/container-builder/docs/ Google Cloud Container Builder API Reference> for @cloudbuild.projects.builds.retry@.
module Network.Google.Resource.Cloudbuild.Projects.Builds.Retry
    (
    -- * REST Resource
...
data ProjectsBuildsRetry = ProjectsBuildsRetry'
    ...
    , _pbrBearerToken :: !(Maybe Text)
    , _pbrId :: !Text
    , _pbrProjectId :: !Text
    , _pbrFields :: !(Maybe Text)
...
pbrPp :: Lens' ProjectsBuildsRetry Bool
pbrPp = lens _pbrPp (\ s a -> s{_pbrPp = a})
...
-- | OAuth bearer token.
pbrBearerToken :: Lens' ProjectsBuildsRetry (Maybe Text)
pbrBearerToken
  = lens _pbrBearerToken
      (\ s a -> s{_pbrBearerToken = a})
...
pbrFields :: Lens' ProjectsBuildsRetry (Maybe Text)
pbrFields
  = lens _pbrFields (\ s a -> s{_pbrFields = a})

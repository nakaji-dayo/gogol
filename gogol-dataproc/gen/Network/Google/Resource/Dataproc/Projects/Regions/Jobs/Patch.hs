...
--
-- /See:/ <https://cloud.google.com/dataproc/ Google Cloud Dataproc API Reference> for @dataproc.projects.regions.jobs.patch@.
module Network.Google.Resource.Dataproc.Projects.Regions.Jobs.Patch
    (
    -- * REST Resource
...
data ProjectsRegionsJobsPatch = ProjectsRegionsJobsPatch'
    ...
    , _prjpPayload :: !Job
    , _prjpBearerToken :: !(Maybe Text)
    , _prjpRegion :: !Text
    , _prjpProjectId :: !Text
    , _prjpFields :: !(Maybe Text)
...
prjpPp :: Lens' ProjectsRegionsJobsPatch Bool
prjpPp = lens _prjpPp (\ s a -> s{_prjpPp = a})
...
-- | OAuth bearer token.
prjpBearerToken :: Lens' ProjectsRegionsJobsPatch (Maybe Text)
prjpBearerToken
  = lens _prjpBearerToken
      (\ s a -> s{_prjpBearerToken = a})
...
prjpFields :: Lens' ProjectsRegionsJobsPatch (Maybe Text)
prjpFields
  = lens _prjpFields (\ s a -> s{_prjpFields = a})

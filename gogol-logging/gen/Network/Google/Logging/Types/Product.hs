...
-- 99% of low-severity log entries from load balancers:
-- \"resource.type=http_load_balancer severity
...
-- then Stackdriver Logging considers other log entries in the same
...
-- in the past, and no more than 24 hours in the future. See the
...
-- For Stackdriver Trace spans, this is the same format that the

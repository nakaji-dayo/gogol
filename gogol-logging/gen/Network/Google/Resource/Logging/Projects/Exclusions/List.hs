...
pelPp :: Lens' ProjectsExclusionsList Bool
pelPp = lens _pelPp (\ s a -> s{_pelPp = a})
...
-- | OAuth bearer token.
pelBearerToken :: Lens' ProjectsExclusionsList (Maybe Text)
pelBearerToken
  = lens _pelBearerToken
      (\ s a -> s{_pelBearerToken = a})
...
pelFields :: Lens' ProjectsExclusionsList (Maybe Text)
pelFields
  = lens _pelFields (\ s a -> s{_pelFields = a})

...
oelPp :: Lens' OrganizationsExclusionsList Bool
oelPp = lens _oelPp (\ s a -> s{_oelPp = a})
...
-- | OAuth bearer token.
oelBearerToken :: Lens' OrganizationsExclusionsList (Maybe Text)
oelBearerToken
  = lens _oelBearerToken
      (\ s a -> s{_oelBearerToken = a})
...
oelFields :: Lens' OrganizationsExclusionsList (Maybe Text)
oelFields
  = lens _oelFields (\ s a -> s{_oelFields = a})

...
data FoldersSinksPatch = FoldersSinksPatch'
    ...
    , _fspPayload :: !LogSink
    , _fspBearerToken :: !(Maybe Text)
    , _fspSinkName :: !Text
    , _fspFields :: !(Maybe Text)
...
fspPp :: Lens' FoldersSinksPatch Bool
fspPp = lens _fspPp (\ s a -> s{_fspPp = a})
...
-- | OAuth bearer token.
fspBearerToken :: Lens' FoldersSinksPatch (Maybe Text)
fspBearerToken
  = lens _fspBearerToken
      (\ s a -> s{_fspBearerToken = a})
...
fspFields :: Lens' FoldersSinksPatch (Maybe Text)
fspFields
  = lens _fspFields (\ s a -> s{_fspFields = a})

...
ePp :: Lens' ExclusionsList Bool
ePp = lens _ePp (\ s a -> s{_ePp = a})
...
-- | OAuth bearer token.
eBearerToken :: Lens' ExclusionsList (Maybe Text)
eBearerToken
  = lens _eBearerToken (\ s a -> s{_eBearerToken = a})
...
eFields :: Lens' ExclusionsList (Maybe Text)
...
instance GoogleRequest ExclusionsList where
        ...
        requestClient ExclusionsList'{..}
          = go _eParent _eXgafv _eUploadProtocol (Just _ePp)
              _eAccessToken
              _eUploadType
              _eBearerToken
              _ePageToken

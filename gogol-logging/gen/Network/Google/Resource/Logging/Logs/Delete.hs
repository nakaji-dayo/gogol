...
ldPp :: Lens' LogsDelete Bool
ldPp = lens _ldPp (\ s a -> s{_ldPp = a})
...
-- | OAuth bearer token.
ldBearerToken :: Lens' LogsDelete (Maybe Text)
ldBearerToken
  = lens _ldBearerToken
      (\ s a -> s{_ldBearerToken = a})
...
ldFields :: Lens' LogsDelete (Maybe Text)

...
-- update are \`labels\`.
--
-- /See:/ <https://cloud.google.com/ml/ Google Cloud Machine Learning Engine Reference> for @ml.projects.jobs.patch@.
module Network.Google.Resource.Ml.Projects.Jobs.Patch
    (
    -- * REST Resource
...
data ProjectsJobsPatch = ProjectsJobsPatch'
    ...
    , _pjpBearerToken :: !(Maybe Text)
    , _pjpName :: !Text
    , _pjpFields :: !(Maybe Text)
...
pjpPp :: Lens' ProjectsJobsPatch Bool
pjpPp = lens _pjpPp (\ s a -> s{_pjpPp = a})
...
-- | OAuth bearer token.
pjpBearerToken :: Lens' ProjectsJobsPatch (Maybe Text)
pjpBearerToken
  = lens _pjpBearerToken
      (\ s a -> s{_pjpBearerToken = a})
...
pjpFields :: Lens' ProjectsJobsPatch (Maybe Text)
pjpFields
  = lens _pjpFields (\ s a -> s{_pjpFields = a})

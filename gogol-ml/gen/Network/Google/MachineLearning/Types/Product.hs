...
data GoogleIAMV1__AuditConfig = GoogleIAMV1__AuditConfig'
    ...
    , _givacExemptedMembers :: !(Maybe [Text])
...
givacExemptedMembers :: Lens' GoogleIAMV1__AuditConfig [Text]
givacExemptedMembers
  = lens _givacExemptedMembers
      (\ s a -> s{_givacExemptedMembers = a})
      . _Default
      . _Coerce
          ...
          = object
              (catMaybes
                 [("service" .=) <$> _givacService,
                  ...
                  ("exemptedMembers" .=) <$> _givacExemptedMembers])
...
-- </ml-engine/docs/how-tos/resource-labels using labels>.
...
givpIAMOwned :: Lens' GoogleIAMV1__Policy (Maybe Bool)
givpIAMOwned
  = lens _givpIAMOwned (\ s a -> s{_givpIAMOwned = a})
...
-- documentation on </ml-engine/docs/how-tos/resource-labels using labels>.
...
-- [standard_p100]
--     A machine equivalent to 'standard' that also includes a single
--     NVIDIA Tesla P100 GPU. The availability of these GPUs is in the
--     Alpha launch stage.

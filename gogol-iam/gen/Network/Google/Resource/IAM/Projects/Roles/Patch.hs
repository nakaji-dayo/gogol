...
--
-- /See:/ <https://cloud.google.com/iam/ Google Identity and Access Management (IAM) API Reference> for @iam.projects.roles.patch@.
module Network.Google.Resource.IAM.Projects.Roles.Patch
    (
    -- * REST Resource
...
prpPp :: Lens' ProjectsRolesPatch Bool
...
-- | OAuth bearer token.
prpBearerToken :: Lens' ProjectsRolesPatch (Maybe Text)
prpBearerToken
...
prpFields :: Lens' ProjectsRolesPatch (Maybe Text)
prpFields
  = lens _prpFields (\ s a -> s{_prpFields = a})

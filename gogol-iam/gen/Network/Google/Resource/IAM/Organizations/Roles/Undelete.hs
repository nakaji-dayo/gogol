...
--
-- /See:/ <https://cloud.google.com/iam/ Google Identity and Access Management (IAM) API Reference> for @iam.organizations.roles.undelete@.
module Network.Google.Resource.IAM.Organizations.Roles.Undelete
    (
    -- * REST Resource
...
data OrganizationsRolesUndelete = OrganizationsRolesUndelete'
    ...
    , _oruBearerToken :: !(Maybe Text)
    , _oruName :: !Text
    , _oruFields :: !(Maybe Text)
...
oruPp :: Lens' OrganizationsRolesUndelete Bool
oruPp = lens _oruPp (\ s a -> s{_oruPp = a})
...
-- | OAuth bearer token.
oruBearerToken :: Lens' OrganizationsRolesUndelete (Maybe Text)
oruBearerToken
  = lens _oruBearerToken
      (\ s a -> s{_oruBearerToken = a})
...
oruFields :: Lens' OrganizationsRolesUndelete (Maybe Text)
oruFields
  = lens _oruFields (\ s a -> s{_oruFields = a})

...
-- Creates a new Role.
--
-- /See:/ <https://cloud.google.com/iam/ Google Identity and Access Management (IAM) API Reference> for @iam.organizations.roles.create@.
module Network.Google.Resource.IAM.Organizations.Roles.Create
    (
    -- * REST Resource
...
data OrganizationsRolesCreate = OrganizationsRolesCreate'
    ...
    , _orcBearerToken :: !(Maybe Text)
    , _orcFields :: !(Maybe Text)
...
orcPp :: Lens' OrganizationsRolesCreate Bool
orcPp = lens _orcPp (\ s a -> s{_orcPp = a})
...
-- | OAuth bearer token.
orcBearerToken :: Lens' OrganizationsRolesCreate (Maybe Text)
orcBearerToken
  = lens _orcBearerToken
      (\ s a -> s{_orcBearerToken = a})

-- | Selector specifying which fields to include in a partial response.
orcFields :: Lens' OrganizationsRolesCreate (Maybe Text)
orcFields
  = lens _orcFields (\ s a -> s{_orcFields = a})

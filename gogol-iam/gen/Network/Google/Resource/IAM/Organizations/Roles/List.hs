...
--
-- /See:/ <https://cloud.google.com/iam/ Google Identity and Access Management (IAM) API Reference> for @iam.organizations.roles.list@.
module Network.Google.Resource.IAM.Organizations.Roles.List
    (
    -- * REST Resource
...
orlPp :: Lens' OrganizationsRolesList Bool
orlPp = lens _orlPp (\ s a -> s{_orlPp = a})
...
-- | OAuth bearer token.
orlBearerToken :: Lens' OrganizationsRolesList (Maybe Text)
orlBearerToken
  = lens _orlBearerToken
      (\ s a -> s{_orlBearerToken = a})
...
orlFields :: Lens' OrganizationsRolesList (Maybe Text)
orlFields
  = lens _orlFields (\ s a -> s{_orlFields = a})

...
--
-- /See:/ <https://cloud.google.com/iam/ Google Identity and Access Management (IAM) API Reference> for @iam.organizations.roles.delete@.
module Network.Google.Resource.IAM.Organizations.Roles.Delete
    (
    -- * REST Resource
...
ordPp :: Lens' OrganizationsRolesDelete Bool
ordPp = lens _ordPp (\ s a -> s{_ordPp = a})
...
-- | OAuth bearer token.
ordBearerToken :: Lens' OrganizationsRolesDelete (Maybe Text)
ordBearerToken
  = lens _ordBearerToken
      (\ s a -> s{_ordBearerToken = a})
...
ordFields :: Lens' OrganizationsRolesDelete (Maybe Text)
ordFields

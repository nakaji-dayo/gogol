...
--
-- Returns the details for the specified interconnect location. Get a list
...
ilgFields :: Lens' InterconnectLocationsGet (Maybe Text)
ilgFields
  = lens _ilgFields (\ s a -> s{_ilgFields = a})
...
instance GoogleRequest InterconnectLocationsGet where
        ...
        requestClient InterconnectLocationsGet'{..}
          = go _ilgProject _ilgInterconnectLocation _ilgFields
              (Just AltJSON)
              computeService
          where go
                  = buildClient

...
pdxrFields :: Lens' ProjectsDisableXpnResource (Maybe Text)
pdxrFields
  = lens _pdxrFields (\ s a -> s{_pdxrFields = a})
...
instance GoogleRequest ProjectsDisableXpnResource
        ...
        requestClient ProjectsDisableXpnResource'{..}
          = go _pdxrProject _pdxrRequestId _pdxrFields
              (Just AltJSON)
              _pdxrPayload
              computeService
          where go

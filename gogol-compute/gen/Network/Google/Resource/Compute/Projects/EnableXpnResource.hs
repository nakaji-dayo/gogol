...
pexrFields :: Lens' ProjectsEnableXpnResource (Maybe Text)
pexrFields
  = lens _pexrFields (\ s a -> s{_pexrFields = a})
...
instance GoogleRequest ProjectsEnableXpnResource
        ...
        requestClient ProjectsEnableXpnResource'{..}
          = go _pexrProject _pexrRequestId _pexrFields
              (Just AltJSON)
              _pexrPayload
              computeService
          where go

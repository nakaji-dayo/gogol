...
iaiFields :: Lens' InterconnectAttachmentsInsert (Maybe Text)
iaiFields
  = lens _iaiFields (\ s a -> s{_iaiFields = a})
...
instance GoogleRequest InterconnectAttachmentsInsert
        ...
        requestClient InterconnectAttachmentsInsert'{..}
          = go _iaiProject _iaiRegion _iaiRequestId _iaiFields
              (Just AltJSON)
              _iaiPayload
              computeService
          where go

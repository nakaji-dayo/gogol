...
napFields :: Lens' NetworksAddPeering (Maybe Text)
napFields
  = lens _napFields (\ s a -> s{_napFields = a})
...
instance GoogleRequest NetworksAddPeering where
        ...
        requestClient NetworksAddPeering'{..}
          = go _napProject _napNetwork _napRequestId _napFields
              (Just AltJSON)
              _napPayload
              computeService
          where go

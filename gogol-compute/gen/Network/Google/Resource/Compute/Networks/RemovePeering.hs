...
nrpFields :: Lens' NetworksRemovePeering (Maybe Text)
nrpFields
  = lens _nrpFields (\ s a -> s{_nrpFields = a})
...
instance GoogleRequest NetworksRemovePeering where
        ...
        requestClient NetworksRemovePeering'{..}
          = go _nrpProject _nrpNetwork _nrpRequestId _nrpFields
              (Just AltJSON)
              _nrpPayload
              computeService
          where go

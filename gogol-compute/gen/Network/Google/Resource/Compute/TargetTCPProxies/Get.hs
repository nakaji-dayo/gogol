...
ttpgFields :: Lens' TargetTCPProxiesGet (Maybe Text)
ttpgFields
  = lens _ttpgFields (\ s a -> s{_ttpgFields = a})
...
instance GoogleRequest TargetTCPProxiesGet where
        ...
        requestClient TargetTCPProxiesGet'{..}
          = go _ttpgProject _ttpgTargetTCPProxy _ttpgFields
              (Just AltJSON)
              computeService
          where go
                  = buildClient

...
apdPp :: Lens' AccountsProductsDelete Bool
apdPp = lens _apdPp (\ s a -> s{_apdPp = a})
...
-- | OAuth bearer token.
apdBearerToken :: Lens' AccountsProductsDelete (Maybe Text)
apdBearerToken
  = lens _apdBearerToken
      (\ s a -> s{_apdBearerToken = a})
...
apdFields :: Lens' AccountsProductsDelete (Maybe Text)
apdFields

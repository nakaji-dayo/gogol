...
alpuFields :: Lens' AdvertiserLandingPagesUpdate (Maybe Text)
alpuFields
  = lens _alpuFields (\ s a -> s{_alpuFields = a})
...
instance GoogleRequest AdvertiserLandingPagesUpdate
        ...
        requestClient AdvertiserLandingPagesUpdate'{..}
          = go _alpuProFileId _alpuFields (Just AltJSON)
              _alpuPayload
              dFAReportingService
          where go
                  = buildClient

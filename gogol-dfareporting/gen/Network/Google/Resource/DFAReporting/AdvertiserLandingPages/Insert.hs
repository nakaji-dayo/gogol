...
alpiFields :: Lens' AdvertiserLandingPagesInsert (Maybe Text)
alpiFields
  = lens _alpiFields (\ s a -> s{_alpiFields = a})
...
instance GoogleRequest AdvertiserLandingPagesInsert
        ...
        requestClient AdvertiserLandingPagesInsert'{..}
          = go _alpiProFileId _alpiFields (Just AltJSON)
              _alpiPayload
              dFAReportingService
          where go
                  = buildClient

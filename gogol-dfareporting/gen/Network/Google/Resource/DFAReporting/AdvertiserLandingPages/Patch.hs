...
alppFields :: Lens' AdvertiserLandingPagesPatch (Maybe Text)
alppFields
  = lens _alppFields (\ s a -> s{_alppFields = a})
...
instance GoogleRequest AdvertiserLandingPagesPatch
        ...
        requestClient AdvertiserLandingPagesPatch'{..}
          = go _alppProFileId (Just _alppId) _alppFields
              (Just AltJSON)
              _alppPayload
              dFAReportingService
          where go

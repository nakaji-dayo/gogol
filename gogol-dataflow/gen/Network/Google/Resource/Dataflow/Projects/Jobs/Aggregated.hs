...
--
-- /See:/ <https://cloud.google.com/dataflow Google Dataflow API Reference> for @dataflow.projects.jobs.aggregated@.
module Network.Google.Resource.Dataflow.Projects.Jobs.Aggregated
    (
    -- * REST Resource
...
pjaPp :: Lens' ProjectsJobsAggregated Bool
pjaPp = lens _pjaPp (\ s a -> s{_pjaPp = a})
...
-- | OAuth bearer token.
pjaBearerToken :: Lens' ProjectsJobsAggregated (Maybe Text)
pjaBearerToken
  = lens _pjaBearerToken
      (\ s a -> s{_pjaBearerToken = a})
...
pjaFields :: Lens' ProjectsJobsAggregated (Maybe Text)
pjaFields
  = lens _pjaFields (\ s a -> s{_pjaFields = a})

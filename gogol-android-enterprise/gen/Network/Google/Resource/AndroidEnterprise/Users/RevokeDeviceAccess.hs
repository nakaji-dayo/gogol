...
urdaFields :: Lens' UsersRevokeDeviceAccess (Maybe Text)
urdaFields
  = lens _urdaFields (\ s a -> s{_urdaFields = a})
...
instance GoogleRequest UsersRevokeDeviceAccess where
        ...
        requestClient UsersRevokeDeviceAccess'{..}
          = go _urdaEnterpriseId _urdaUserId _urdaFields
              (Just AltJSON)

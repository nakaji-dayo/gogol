...
--
-- Demotes the standalone instance to be a read replica Cloud SQL instance
-- of an on-premises master.
--
-- /See:/ <https://cloud.google.com/sql/docs/reference/latest Cloud SQL Administration API Reference> for @sql.instances.demoteMaster@.
module Network.Google.Resource.SQL.Instances.DemoteMaster
    (
    -- * REST Resource
...
idmFields :: Lens' InstancesDemoteMaster (Maybe Text)
idmFields
  = lens _idmFields (\ s a -> s{_idmFields = a})
...
instance GoogleRequest InstancesDemoteMaster where
        ...
        requestClient InstancesDemoteMaster'{..}
          = go _idmProject _idmInstance _idmFields
              (Just AltJSON)
              _idmPayload
              sQLAdminService
          where go

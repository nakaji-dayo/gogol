...
calaPp :: Lens' CoursesAliasesList Bool
calaPp = lens _calaPp (\ s a -> s{_calaPp = a})
...
-- | OAuth bearer token.
calaBearerToken :: Lens' CoursesAliasesList (Maybe Text)
calaBearerToken
  = lens _calaBearerToken
      (\ s a -> s{_calaBearerToken = a})
...
calaFields :: Lens' CoursesAliasesList (Maybe Text)
calaFields
  = lens _calaFields (\ s a -> s{_calaFields = a})

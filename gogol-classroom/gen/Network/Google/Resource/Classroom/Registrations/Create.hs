...
-- notifications from the provided \`feed\` to the provided
-- \`destination\`. Returns the created \`Registration\`. Currently, this
-- will be the same as the argument, but with server-assigned fields such
-- as \`expiry_time\` and \`id\` filled in. Note that any value specified
-- for the \`expiry_time\` or \`id\` fields will be ignored. While
-- Classroom may validate the \`destination\` and return errors on a best
-- effort basis, it is the caller\'s responsibility to ensure that it
-- exists and that Classroom has permission to publish to it. This method
-- may return the following error codes: * \`PERMISSION_DENIED\` if: * the
-- authenticated user does not have permission to receive notifications
-- from the requested field; or * the credential provided does not include
-- the appropriate scope for the requested feed. * another access error is
-- encountered. * \`INVALID_ARGUMENT\` if: * no \`destination\` is
-- specified, or the specified \`destination\` is not valid; or * no
-- \`feed\` is specified, or the specified \`feed\` is not valid. *
...
-- exists; or * the specified \`destination\` cannot be located, or
...
rcPp :: Lens' RegistrationsCreate Bool
rcPp = lens _rcPp (\ s a -> s{_rcPp = a})
...
-- | OAuth bearer token.
rcBearerToken :: Lens' RegistrationsCreate (Maybe Text)
rcBearerToken
  = lens _rcBearerToken
      (\ s a -> s{_rcBearerToken = a})

-- | Selector specifying which fields to include in a partial response.
rcFields :: Lens' RegistrationsCreate (Maybe Text)
...
instance GoogleRequest RegistrationsCreate where
        ...
        requestClient RegistrationsCreate'{..}
          = go _rcXgafv _rcUploadProtocol (Just _rcPp)
              _rcAccessToken
              _rcUploadType
              _rcBearerToken
              _rcCallback

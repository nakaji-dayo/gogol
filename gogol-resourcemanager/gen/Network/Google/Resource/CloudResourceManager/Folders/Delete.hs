...
--
-- /See:/ <https://cloud.google.com/resource-manager Google Cloud Resource Manager API Reference> for @cloudresourcemanager.folders.delete@.
module Network.Google.Resource.CloudResourceManager.Folders.Delete
    (
    -- * REST Resource
...
fdPp :: Lens' FoldersDelete Bool
fdPp = lens _fdPp (\ s a -> s{_fdPp = a})
...
-- | OAuth bearer token.
fdBearerToken :: Lens' FoldersDelete (Maybe Text)
fdBearerToken
  = lens _fdBearerToken
      (\ s a -> s{_fdBearerToken = a})
...
fdFields :: Lens' FoldersDelete (Maybe Text)
...
instance GoogleRequest FoldersDelete where
        ...
        requestClient FoldersDelete'{..}
          = go _fdName _fdXgafv _fdUploadProtocol (Just _fdPp)
              _fdAccessToken
              _fdUploadType
              _fdBearerToken
              _fdCallback

...
--
-- /See:/ <https://cloud.google.com/resource-manager Google Cloud Resource Manager API Reference> for @cloudresourcemanager.folders.search@.
module Network.Google.Resource.CloudResourceManager.Folders.Search
    (
    -- * REST Resource
...
data FoldersSearch = FoldersSearch'
    ...
    , _fsBearerToken :: !(Maybe Text)
    , _fsFields :: !(Maybe Text)
...
fsPp :: Lens' FoldersSearch Bool
fsPp = lens _fsPp (\ s a -> s{_fsPp = a})
...
-- | OAuth bearer token.
fsBearerToken :: Lens' FoldersSearch (Maybe Text)
fsBearerToken
  = lens _fsBearerToken
      (\ s a -> s{_fsBearerToken = a})

-- | Selector specifying which fields to include in a partial response.
fsFields :: Lens' FoldersSearch (Maybe Text)
fsFields = lens _fsFields (\ s a -> s{_fsFields = a})
...
instance GoogleRequest FoldersSearch where
        ...
        requestClient FoldersSearch'{..}
          = go _fsXgafv _fsUploadProtocol (Just _fsPp)
              _fsAccessToken
              _fsUploadType
              _fsBearerToken
              _fsCallback

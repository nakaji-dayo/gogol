...
-- call.
--
-- /See:/ <https://cloud.google.com/resource-manager Google Cloud Resource Manager API Reference> for @cloudresourcemanager.folders.testIamPermissions@.
module Network.Google.Resource.CloudResourceManager.Folders.TestIAMPermissions
    (
    -- * REST Resource
...
data FoldersTestIAMPermissions = FoldersTestIAMPermissions'
    ...
    , _ftipBearerToken :: !(Maybe Text)
    , _ftipResource :: !Text
    , _ftipFields :: !(Maybe Text)
...
ftipPp :: Lens' FoldersTestIAMPermissions Bool
ftipPp = lens _ftipPp (\ s a -> s{_ftipPp = a})
...
-- | OAuth bearer token.
ftipBearerToken :: Lens' FoldersTestIAMPermissions (Maybe Text)
ftipBearerToken
  = lens _ftipBearerToken
      (\ s a -> s{_ftipBearerToken = a})
...
ftipFields :: Lens' FoldersTestIAMPermissions (Maybe Text)
ftipFields
  = lens _ftipFields (\ s a -> s{_ftipFields = a})
